...
class TransportService:
    ...
    @staticmethod
    async def add_forward_group(bot: Bot, group_id: int) -> Optional[str]:
        """添加转发群组
        ...
        # 检查是否已在转发列表中
        forward_groups = TransportService.get_forward_groups()
        if any(group["group_id"] == group_id for group in forward_groups):
            return f"群 {group_id} 已经在转发列表中"
    ...
    @staticmethod
    def remove_forward_group(group_id: int) -> Optional[str]:
        """移除转发群组
...
async def handle_transport(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    try:
        ...
        if command in ["list", "ls", "查看", "查询"]:
            ...
            error_msg = await TransportService.add_forward_group(bot, group_id)
            ...
            group_id = int(params[1])
            error_msg = TransportService.remove_forward_group(group_id)
