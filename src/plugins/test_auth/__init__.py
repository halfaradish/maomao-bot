...
async def _is_allowed(event: GroupMessageEvent, perm_key: str = "member:ban") -> bool:
    """
    ...
    if siqi_auth.enabled:
        try:
            return await siqi_auth.check(perm_key, user_id)
        except Exception as e:
            logger.warning(f"[auth_plugin] 司契权限系统调用失败，降级到本地白名单: {e}")
            return _is_allowed_local(event)
...
async def handle_ban(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        ...
        if event.user_id in driver.config.superusers:
            ...
            if siqi_auth.enabled:
                try:
                    allowed, latency = await siqi_auth.check_with_latency("member:ban", str(event.user_id))
                except Exception as e:
        ...
        if not allowed:
            await ban_cmd.finish(f"你没有权限使用禁言功能 (权限检查延迟 {latency:.2f}ms)")
            return
        ...
        try:
            ...
            # 添加权限检查延迟信息到返回消息
            await ban_cmd.finish(f"已禁言 {target_user}，时长 {duration} 秒 (权限检查延迟 {latency:.2f}ms)")
...
async def handle_unban(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        ...
        if not allowed:
            await unban_cmd.finish(f"你没有权限使用解除禁言功能 (权限检查延迟 {latency:.2f}ms)")
            return
        ...
        try:
            ...
            # 添加权限检查延迟信息到返回消息
            await unban_cmd.finish(f"已解除 {target_user} 的禁言 (权限检查延迟 {latency:.2f}ms)")
        except FinishedException:
            raise
        except Exception as e:
...
async def handle_kick(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        ...
        if event.user_id in driver.config.superusers:
            ...
            if siqi_auth.enabled:
                try:
                    allowed, latency = await siqi_auth.check_with_latency("member:kick", str(event.user_id))
                except Exception as e:
                    logger.warning(f"[auth_plugin] 权限系统调用失败，降级到本地白名单: {e}")
                    allowed = _is_allowed_local(event)
                    latency = 0.0
        ...
        if not allowed:
            await kick_cmd.finish(f"你没有权限使用踢人功能 (权限检查延迟 {latency:.2f}ms)")
            return
        ...
        try:
            ...
            # 添加权限检查延迟信息到返回消息
            await kick_cmd.finish(f"已将 {target_user} 移出群聊 (权限检查延迟 {latency:.2f}ms)")
        except FinishedException:
            raise
        except Exception as e:
