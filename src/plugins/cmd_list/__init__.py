...
def _generate_cmd_list_image():
    all_commands = [
        ...
        {"category": " 实用功能", "commands": ["/考勤 --- 查看考勤情况", "/clipboard --- 剪贴板功能", "/cvmd --- Markdown剪贴板功能", "/赞我 --- 给QQ资料卡点赞", "/伪造消息 · 伪消息 --- 生成假消息"]},
        {"category": " 竞赛相关", "commands": ["/contest --- 查看竞赛信息", "/duel --- CF推题", "/开始监控 · monitor · 取消监控 · stop · 监控 --- ICPC过题监控", "/赛时过题 · 过题情况 --- 查询赛时过题情况", "/过题^ · 过题 --- 记录过题", "/过题积分榜 · 积分榜 --- 查看过题排行榜", "/检查过题 · 过题统计 --- 实时过题查询"]},
        {"category": " 群管理", "commands": ["/ban · unban · kick --- 群成员管理", "/group --- 群管理功能", "/group_pull --- 群消息转发", "/群统计 --- 群统计功能", "/一键退群 --- 批量退群"]},
        {"category": " 技术功能", "commands": ["/sum · summary --- LLM对话记录", "/prd --- PRD功能", "/搬史 · 搬屎 · 转发 --- 搬史功能", "/来点涩图 · 来张涩图 · 来张色图 --- 发送涩图", "/测表格 --- 测试表格生成"]},
        {"category": " 系统管理", "commands": ["/rate · 限速启用 · 限速禁用 · 限速紧急停止 · 限速恢复 · 限速状态 · 限速全局 · 限速按群 --- 限速管理", "/todo --- 待办提醒", "/清除缓存 --- 清除群成员缓存"]}
    ]
    ...
    for category in all_commands:
        _draw_rounded_rect(draw, [padding, current_y, padding + content_width, current_y + category_header_height], 8, '#e8e8e8')
        ...
        for i in range(0, len(commands), items_per_row):
            for j in range(items_per_row):
                ...
                draw.rectangle([x + 2, current_y + 2, x + item_width - 2, current_y + item_height - 2], outline='#000000', width=2)
