...
async function load(pageNum) {
  ...
  try {
    const endpoint = `/permissions/whitelist/${activeTab.value}?page=${page.value}&size=${PAGE_SIZE}`
...
async function doAdd() {
  ...
  try {
    ...
    await apiPost(`/permissions/whitelist/${activeTab.value}`, body)
    showToast('已添加')
    form.id = ''
    form.reason = ''
    await load()
...
async function doDelete(item) {
  ...
  try {
    await showConfirm('移出白名单', `确定将此${label}移出白名单吗？`)
  } catch {
    return
  }
  try {
    await apiDelete(`/permissions/whitelist/${activeTab.value}/${id}`)
    showToast('已移出')
    await load()
  } catch (e) {
